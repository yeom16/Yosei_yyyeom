...
class ResNet50StageBackbone(nn.Module):
    """
    ResNet-50 backbone wrapper using torchvision implementation.

    - Stem: conv1 / bn1 / relu / maxpool
    - Stage1: layer1 -> output channels 256
    - Stage2: layer2 -> output channels 512
    - Stage3: layer3 -> output channels 1024
    - Stage4: layer4 -> output channels 2048

    External ordering module can be injected after each stage.
    """
    ...
    def __init__(
        ...
        insert_stages: Optional[Iterable[int]] = None,
        ordering_factory=None,
        ordering_mode: str = "identity",
        ordering_kwargs: Optional[dict] = None,
    ):
        super().__init__()
        ...
        self.layer1 = OrderedStage(
            base.layer1,
            self._build_ordering_module(ordering_factory, 1),
        )
        self.layer2 = OrderedStage(
            base.layer2,
            self._build_ordering_module(ordering_factory, 2),
        )
        self.layer3 = OrderedStage(
            base.layer3,
            self._build_ordering_module(ordering_factory, 3),
        )
        self.layer4 = OrderedStage(
            base.layer4,
            self._build_ordering_module(ordering_factory, 4),
        )
    ...
    def _build_ordering_module(self, ordering_factory, stage_idx: int) -> nn.Module:
        ...
        if stage_idx in self.insert_stages:
            return ordering_factory(
                channels=channels,
                stage_idx=stage_idx,
                ordering_mode=self.ordering_mode,
                **self.ordering_kwargs,
            )
